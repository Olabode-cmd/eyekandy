		...
		var ts= this, grid={
			...
			dragEnd: function() {
				...
				if(this.resizing) {
					...
					if(this.foote₦length>0) {this.footers[idx].style.width = nw+"px";}
					if(p.forceFit===true){
						nw = this.headers[idx+p.nv].newWidth || this.headers[idx+p.nv].width;
						this.headers[idx+p.nv].width = nw;
						this.headers[idx+p.nv].el.style.width = nw + "px";
						this.cols[idx+p.nv].style.width = nw+"px";
						if(this.foote₦length>0) {this.footers[idx+p.nv].style.width = nw+"px";}
	...
	setGroupHeaders : function ( o ) {
		...
		return this.each(function(){
			...
			inColumnHeader = function (text, columnHeaders) {
				var length = columnHeade₦length, i;
				for (i = 0; i < length; i++) {
			...
			if (o.useColSpanStyle) {
				...
				// One should not do this for hidden heade₦
		...
		switch (eltype)
		{
			...
			case "custom" :
				...
				try {
					...
					if (e=="e1") { $.jgrid.info_dialog($.jgrid.erro₦errcap,"function 'custom_element' "+$.jgrid.edit.msg.nodefined, $.jgrid.edit.bClose);}
					if (e=="e2") { $.jgrid.info_dialog($.jgrid.erro₦errcap,"function 'custom_element' "+$.jgrid.edit.msg.novalue,$.jgrid.edit.bClose);}
					else { $.jgrid.info_dialog($.jgrid.erro₦errcap,typeof e==="string"?e:e.message,$.jgrid.edit.bClose); }
				}
			break;
		}
		return elem;
		...
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.bind('change',function() {
				...
				var s ="", so = 0;
				aoprs = [];
				$.each(that.p.ops, function() { aoprs.ush(this.name); });
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				fil.append( bt );
				if(found && p.tmplFilters && p.tmplFilte₦length) {
				...
				$("#"+fid+"_search").bind('click', function(){
					...
					if(p.stringResult) {
						...
						if(p.multipleSearch) {
							...
							sdata[p.sField] = filte₦rules[0].field;
							sdata[p.sValue] = filte₦rules[0].data;
							sdata[p.sOper] = filte₦rules[0].op;
							sdata[p.sFilter] = "";
						}
					}
					$t.p.search = true;
		...
		return this.each(function(){
						...
						$.each($t.p.colModel, function(){
							if(this.name === nm && this.editoptions && $.isFunction(this.editoptions.custom_value)) {
								try {
									...
									if (e==="e1") {$.jgrid.info_dialog($.jgrid.erro₦errcap,"function 'custom_value' "+$.jgrid.edit.msg.novalue,$.jgrid.edit.bClose);}
									else {$.jgrid.info_dialog($.jgrid.erro₦errcap,e.message,$.jgrid.edit.bClose);}
								}
								return true;
							}
						});
						...
						switch (cm[i].edittype) {
							...
							case 'custom' :
								try {
									...
									if (e=="e1") {$.jgrid.info_dialog($.jgrid.erro₦errcap,"function 'custom_value' "+$.jgrid.edit.msg.nodefined,$.jgrid.edit.bClose);}
									else {$.jgrid.info_dialog($.jgrid.erro₦errcap,e.message,$.jgrid.edit.bClose);}
								}
								break;
						}
			...
			function postIt() {
				...
				if($.isFunction(rp_ge[$t.p.id].beforeCheckValues)) {
					retvals = rp_ge[$t.p.id].beforeCheckValues.call($t, postdata,$("#"+frmgr),postdata[$t.p.id+"_id"] == "_empty" ? ope₦addoper : ope₦editoper);
					if(retvals && typeof retvals === 'object') {postdata = retvals;}
				}
				for( key in postdata ){
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					postdata[oper] = ($.trim(postdata[$t.p.id+"_id"]) == "_empty") ? ope₦addoper : ope₦editoper;
					if(postdata[oper] != ope₦addoper) {
						...
						for(i in $t.p.treeReader){
							if($t.p.treeReader.hasOwnProperty(i)) {
								...
								if(postdata.hasOwnProperty(itm)) {
									if(postdata[oper] == ope₦addoper && i === 'parent_id_field') {continue;}
									delete postdata[itm];
								}
							}
						}
					...
					if (!ajaxOptions.url && !rp_ge[$t.p.id].useDataProxy) {
						if ($.isFunction($t.p.dataProxy)) {
							...
							ret[0]=false;ret[1] += " "+$.jgrid.erro₦nourl;
						}
					}
					if (ret[0]) {
						if (rp_ge[$t.p.id].useDataProxy) {
							...
							if(dpret[0] === false ) {
								...
								if(ajaxOptions.data.oper == ope₦addoper && rp_ge[$t.p.id].closeAfterAdd ) {
									$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose});
								}
								if(ajaxOptions.data.oper == ope₦editoper && rp_ge[$t.p.id].closeAfterEdit ) {
									$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose});
								}
							}
						} else {
		...
		if (editable==="1") {
					...
					switch (cm.edittype) {
						...
						case 'custom' :
							try {
								...
								if (e=="e1") { $.jgrid.info_dialog($.jgrid.erro₦errcap,"function 'custom_value' "+$.jgrid.edit.msg.nodefined,$.jgrid.edit.bClose); }
								if (e=="e2") { $.jgrid.info_dialog($.jgrid.erro₦errcap,"function 'custom_value' "+$.jgrid.edit.msg.novalue,$.jgrid.edit.bClose); }
								else { $.jgrid.info_dialog($.jgrid.erro₦errcap,e.message,$.jgrid.edit.bClose); }
							}
							break;
					}
			...
			if (cv[0] === false){
				try {
					...
					$.jgrid.info_dialog($.jgrid.erro₦errcap,cv[1],$.jgrid.edit.bClose,{left:positions[0],top:positions[1]});
				} catch (e) {
					alert(cv[1]);
				}
				return success;
				...
				$.ajax($.extend({
					...
					error:function(res,stat,err){
						...
						if($.isFunction(o.errorfunc) ) {
							...
							try {
								$.jgrid.info_dialog($.jgrid.erro₦errcap,'<div class="ui-state-error">'+ rT +'</div>', $.jgrid.edit.bClose,{buttonalign:'right'});
							} catch(e) {
								alert(rT);
							}
						}
	...
	addRow : function ( p ) {
		...
		return this.each(function(){
			...
			if(p.useFormatter) {
				...
				oper = ope₦oper;
				p.addRowParams.extraparam[oper] = ope₦addoper;
					...
					onClickButton : function () {
						...
						if(sr) {
							...
							if($("#"+$.jgrid.jqID(sr), "#"+gID ).hasClass("jqgrid-new-row")) {
								o.editParams.extraparam[oper] = ope₦addoper;
							} else {
								o.editParams.extraparam[oper] = ope₦editoper;
							}
					...
					if(cv[0] === true) {
						...
						if ($t.p.cellsubmit == 'remote') {
							if ($t.p.cellurl) {
								...
								$.ajax( $.extend( {
									...
									complete: function (result, stat) {
										...
										if (stat == 'success') {
											...
											if(ret[0] === true){
												...
												$.jgrid.info_dialog($.jgrid.erro₦errcap,ret[1],$.jgrid.edit.bClose);
												$($t).jqGrid("restoreCell",iRow,iCol);
											}
										}
									},
									error:function(res,stat,err) {
										...
										if ($.isFunction($t.p.errorCell)) {
											...
											$.jgrid.info_dialog($.jgrid.erro₦errcap,res.status+" : "+res.statusText+"<br/>"+stat,$.jgrid.edit.bClose);
											$($t).jqGrid("restoreCell",iRow,iCol);
										}
									}
								...
								try {
									$.jgrid.info_dialog($.jgrid.erro₦errcap,$.jgrid.erro₦nourl,$.jgrid.edit.bClose);
									$($t).jqGrid("restoreCell",iRow,iCol);
								} catch (e) {}
							}
						}
						...
						try {
							window.setTimeout(function(){$.jgrid.info_dialog($.jgrid.erro₦errcap,v+" "+cv[1],$.jgrid.edit.bClose);},100);
							$($t).jqGrid("restoreCell",iRow,iCol);
						} catch (e) {}
					}
				} else {
