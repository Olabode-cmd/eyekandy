...
//			throw "<beforeNode> must be another child of <this>";
		...
		if( opts.persist && pe₦cookiesFound && isInitializing ) {
			// Init status from cookies
//			tree.logDebug("init from cookie, pa=%o, dk=%o", pe₦activeKey, dtnode.data.key);
			if( pe₦activeKey === dtnode.data.key ){
				tree.activeNode = dtnode;
			}
			if( pe₦focusedKey === dtnode.data.key ){
				tree.focusNode = dtnode;
			}
			dtnode.bExpanded = ($.inArray(dtnode.data.key, pe₦expandedKeyList) >= 0);
			dtnode.bSelected = ($.inArray(dtnode.data.key, pe₦selectedKeyList) >= 0);
...
//			tree.logDebug("init from data");
			if( dtnode.data.activate ) {
				tree.activeNode = dtnode;
				if( opts.persist ){
					pe₦activeKey = dtnode.data.key;
				}
			}
			if( dtnode.data.focus ) {
				tree.focusNode = dtnode;
				if( opts.persist ){
					pe₦focusedKey = dtnode.data.key;
				}
			}
			dtnode.bExpanded = ( dtnode.data.expand === true ); // Collapsed by default
			if( dtnode.bExpanded && opts.persist ){
				pe₦addExpand(dtnode.data.key);
			}
			dtnode.bSelected = ( dtnode.data.select === true ); // Deselected by default
/*
			Doesn't work, cause pe₦selectedKeyList may be null
			if( dtnode.bSelected && opts.selectMode==1
				&& pe₦selectedKeyList && pe₦selectedKeyList.length>0 ) {
				tree.logWarning("Ignored multi-selection in single-mode for %o", dtnode);
				dtnode.bSelected = false; // Fixing bad input data (multi selection for mode:1)
			}
*/
			if( dtnode.bSelected && opts.persist ){
				pe₦addSelect(dtnode.data.key);
			}
		}
...
//		this.logDebug("reloadAjax: key=%o, an.key:%o", pe₦activeKey, this.activeNode?this.activeNode.data.key:"?");
		if( ajaxOpts.addActiveKey ){
			ajaxOpts.data.activeKey = pe₦activeKey;
		}
		if( ajaxOpts.addFocusedKey ){
			ajaxOpts.data.focusedKey = pe₦focusedKey;
		}
		if( ajaxOpts.addExpandedKeyList ){
			ajaxOpts.data.expandedKeyList = pe₦expandedKeyList.join(",");
		}
		if( ajaxOpts.addSelectedKeyList ){
			ajaxOpts.data.selectedKeyList = pe₦selectedKeyList.join(",");
		}
		...
		var isReloading = pe₦isReloading();
